...
###### READING IN HANNAH'S DATA #######
hannahvaccine = readRDS(here("data","derived","private","vaccineTweets_clean.RDS"))

# joining data for whole country
# this step removes from my data the tweets that overlapped w Hannah's
totalvax = anti_join(vaccine, hannahvaccine, by="status_id")

#now we join this to hannah's 
totalvax2 = full_join(totalvax, hannahvaccine)

saveRDS(totalvax2, here("data","derived","private","totalvaccine.RDS"))
...
vaccineTweetsByHour <- ts_data(totalvax2, by="hours")
ts_plot(totalvax2, by="hours")

############# NETWORK ANALYSIS ############# 

# Create network data frame. 
# Other options for 'edges' in the network include mention, retweet, and reply
vaccineNetwork <- network_graph(totalvax2, c("quote"))
...
vaccineText = totalvax2 %>% select(text) %>% plain_tweets()
...
vaccineWordPairs %>%
  ...
  labs(title = "Word Network of Tweets Regarding the Covid-19 Vaccine",
       x = "", y = "") +
...
ggplot() +
  ...
  labs(title = "Tweet Locations That Mention 'Vaccine'")+
  theme(plot.title=element_text(hjust=0.5),
